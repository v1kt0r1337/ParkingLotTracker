...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
	...
	#If contour area is bigger than our pre-defined object size...
	    ...
	    #This loop draws lines showing vectors of objects between frames
            for i in cars:
                if len(i.getTracks()) >= 2:
                    ...
                    image = cv2.polylines(image,[cts],False,i.getRGB())
                if i.getId() == 9:
		...
		#Draw lines, object counters and text on the video frame
                ...
                frame = cv2.polylines(image, [pts_L1],False,line_down_color,thickness=4)
                frame = cv2.polylines(image, [pts_L2],False,line_up_color,thickness=4)
                frame = cv2.polylines(image, [pts_L3],False,(255,255,255),thickness=2)
                frame = cv2.polylines(image, [pts_L4],False,(255,255,255),thickness=2)
    ...
    #Clears current frame in preperation for next one
